...
class MateriasController extends AppController
{
    ...
    public function disponibilidad($id=null)
    {
        ...
        $profesors1 = $this->Materias->Profesors->find('all', ['conditions' => ['Profesors.contrato' => 'Tiempo Completo']]);
        $profesors2 = $this->Materias->Profesors->find('all', ['conditions' => ['Profesors.contrato' => 'Por Horas']]);
        
        
        foreach ($profesors1 as $profesor)
        {
                if($profesor['materia_id']==$materia['id']) 
                {
                    array_push($profesorst, $profesor);
                } 
            
        }

        foreach ($profesors2 as $profesor)
        {
        
            if($profesor['materia_id']==$materia['id']) 
            {
                array_push($profesorsp, $profesor);
            } 
            
        }       
        $this->set(compact('profesorst', 'profesorsp', 'materia'));
